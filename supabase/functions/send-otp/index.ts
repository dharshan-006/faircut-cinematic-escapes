...
serve(async (req) => {
  ...
  try {
    ...
    await client.connectTLS({
      ...
      password: "swuu nxru wcdb fqdr", // Using the app password provided
    });
    ...
    await client.send({
      ...
      content: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #8B5CF6; border-radius: 10px;">
          ...
          <p style="color: #666; text-align: center; margin-top: 30px; font-size: 12px;">Fair-Cut - No Convenience Fee, Just Movies</p>
        </div>
      `,
      html: true,
    });
    ...
    // In a real app, we would store the OTP with the email in a database table
    // with an expiration time, but for simplicity of this demo we'll return it
    // and let the frontend handle it (not secure but demonstrates the flow)
    return new Response(
      JSON.stringify({ 
        message: "OTP sent successfully", 
        otp: otp // In a real app, NEVER return the OTP to the frontend
      }),
